#!/usr/bin/env python
"""
Batch runner: full preprocessing + windowed spiral detection + sigma sweep.

Key behavior:
- Reads a subject manifest (CSV/TSV/JSON; required column: cifti_file).
- For each subject, performs full preprocessing (full phase cube x/y/t).
- Detects spirals only inside a selected frame window to reduce runtime.
- Uses the same frame window for all sigma groups within a subject.
...
from matphase.detect.spirals import SpiralDetectionResult, SpiralPattern, detect_spirals_directional
...
sigmaband1 = [mm / mm2sigma_scale for mm in mmband1]
sigmaband2 = [mm / mm2sigma_scale for mm in mmband2]
sigmaband3 = [mm / mm2sigma_scale for mm in mmband3]
sigmaband4 = [mm / mm2sigma_scale for mm in mmband4]

print("Sigma bands (in mm):")
for idx, (mm_band, sigma_band) in enumerate(zip([mmband1, mmband2, mmband3, mmband4], [sigmaband1, sigmaband2, sigmaband3, sigmaband4]), start=1):
    print(f"Band {idx}: {mm_band} mm -> sigma scales {sigma_band}")


# ============================ USER CONFIG (edit here) ============================
DEFAULT_CONFIG = Path("configs/defaults.yaml")
DEFAULT_CIFTI_FILE = Path("")
DEFAULT_OUTDIR = Path("output/sigma_sweep")
# Each inner list is one DoG sigma_scales group for one run.
DEFAULT_SIGMA_SCALE_GROUPS: List[List[float]] = [
    sigmaband1,
    sigmaband2,
    sigmaband3,
    sigmaband4,
...
def _parse_sigma_groups(raw_groups: Optional[List[List[float]]]) -> List[List[float]]:
    groups = raw_groups if raw_groups else DEFAULT_SIGMA_SCALE_GROUPS
    if not groups:
        raise ValueError(
            "No sigma groups available. Set DEFAULT_SIGMA_SCALE_GROUPS in this script "
            "or provide --sigma-scales-group."
        )
    parsed: List[List[float]] = []
    for idx, grp in enumerate(groups, start=1):
        vals = [float(x) for x in grp]
        if not vals or any(v <= 0 for v in vals):
            raise ValueError(f"Invalid sigma group #{idx}: {grp}")
        parsed.append(vals)
    return parsed
...
def _format_sigma_tag(scales: Sequence[float]) -> str:
    tokens = [f"{v:.6g}".replace("-", "m").replace(".", "p") for v in scales]
    return "sigma_" + "__".join(tokens)
...
def run_pipeline_for_hemisphere_windowed(
    *,
    ...
    # Full phase cube is always computed/saved for correctness.
    ...
    if config.output.save_phase_cube and phase_cube_output_path is not None:
        ...
        np.save(phase_cube_output_path, phase_cube_full)
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Batch sigma-sweep runner with full preprocessing and windowed detection.",
    )
    ...
    parser.add_argument("--curl-threshold", type=float, default=None)
    ...
    parser.add_argument(
        ...
        type=float,
        required=False,
        help="Optional override. Repeat for multiple groups; if omitted, uses DEFAULT_SIGMA_SCALE_GROUPS in this script.",
    )
...
def _process_subject(
    ...
    row: Dict[str, str],
    sigma_groups: Sequence[Sequence[float]],
) -> List[Path]:
    ...
    def _bundle_complete(bundle_dir: Path) -> bool:
        required = [
    ...
    saved: List[Path] = []
    for sigma_group in sigma_groups:
        config = load_config(args.config, apply_env=True)
        _apply_common_overrides(config, args)
        config.preprocessing.sigma_scales = [float(v) for v in sigma_group]
        config.paths.cifti_file = str(cifti_path)

        cifti_ts = load_cifti(str(cifti_path))
        metadata_sampling_rate = getattr(
            cifti_ts.metadata, "sampling_rate", None)
        ...
        sampling_rate_hz = _resolve_sampling_rate(
            config, metadata_sampling_rate)

        sigma_root = subject_root / _format_sigma_tag(sigma_group)
        sigma_root.mkdir(parents=True, exist_ok=True)

        # Skip this sigma group if all target hemisphere bundles are complete.
        ...
        for hemi_sel in hemispheres:
            ...
            target_bundle_dirs[hemi_sel] = sigma_root / bundle_name
        if all(_bundle_complete(p) for p in target_bundle_dirs.values()):
            logger.info(
                "Skip subject=%s sigma=%s (existing complete bundles found).",
                subject_key,
                _format_sigma_tag(sigma_group),
            )
        ...
        for hemi_sel in hemispheres:
            ...
            res, surrogate = run_pipeline_for_hemisphere_windowed(
                hemisphere=hemi_sel,
                config=config,
            ...
            surrogate_by_hemi[hemi_sel] = surrogate
...
def _build_subject_command(args: argparse.Namespace, row: Dict[str, str]) -> List[str]:
    ...
    if args.curl_threshold is not None:
        cmd += ["--curl-threshold", str(args.curl_threshold)]
    if args.min_duration is not None:
    ...
    if args.sigma_scales_group:
        for grp in args.sigma_scales_group:
            cmd += ["--sigma-scales-group", *[str(v) for v in grp]]
...
def main() -> int:
    args = _parse_args()
    sigma_groups = _parse_sigma_groups(args.sigma_scales_group)

    args.outdir.mkdir(parents=True, exist_ok=True)
    log_path = args.log_file if args.log_file is not None else (
        args.outdir / "run_windowed_sigma_batch.log")
    ...
    if args.single_cifti_file:
        ...
        saved = _process_subject(args, row, sigma_groups)
